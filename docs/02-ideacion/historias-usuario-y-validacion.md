...
**Alcance confirmado, no ampliar en esta iteración (05 Set., a pedido explícito del equipo):** estos 3 usos son el límite del MVP. Se evaluó explícitamente ampliar el agente a soporte general de consultas (beneficios, seguros, "otros apartados") y **se decidió no hacerlo ahora**, por 3 razones: (a) ninguno de los dos mentores lo tocó ni fue parte de las 4 preguntas preparadas (`hallazgos-mentoria-04-set.md`, gap ya registrado en `CONTEXTO-ACTUAL.md`) — es una extensión sin ninguna señal de demanda real, justo cuando el objetivo es subir el rigor de validación, no bajarlo; (b) contradice el principio ya cerrado de "prototipo funcional acotado, no app para masas" — un agente de soporte general multiplica la superficie de fallo en una demo de 3 minutos; (c) diluye el foco del pitch — el diferencial real de FIBO es el mecanismo hábito→Reserva→recompensa→microseguro, no un chatbot de atención al cliente, que además ya tiene canales existentes en Pacífico (Dr. Online, Susalud, asesores). **Se documenta como extensión de producto a mediano plazo** (mismo tratamiento que comunidades/seguro grupal y "dejar malos hábitos", §8) — validar primero con campo/mentoría antes de construirla.
...
## 8. Visión de producto completa vs. alcance del MVP de hackathon (04-05 Set.)

Tres extensiones de producto quedaron fuera del MVP de 7 días — dos porque salieron validadas en la mentoría pero exceden el alcance (`hallazgos-mentoria-04-set.md` §3.3-3.4), una porque el propio equipo evaluó ampliarla y decidió no hacerlo todavía (§6 arriba). En los tres casos el criterio es el mismo, ya cerrado por el equipo: el hackathon construye un prototipo funcional acotado, no una aplicación lista para escala masiva:
...
3. **El agente conversacional como soporte general de consultas** (beneficios, seguros, "otros apartados") — evaluado y descartado para esta iteración por decisión explícita del equipo (05 Set., ver §6 arriba); el agente del MVP se mantiene acotado a onboarding y primer filtro del momento de verdad.
